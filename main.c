...
//get ipro surface area and edit grid score (skip ipro->atoms[].sa < sat)
int main(int argc, char *argv[]) {

	char results_path[100], details_path[100];
	struct stat sb;

	checkinput(fscanf(stdin, "RESULTS_PATH: %s\n", results_path));
	checkinput(fscanf(stdin, "DETAILS_PATH: %s\n", details_path));

	if (stat(results_path, &sb) == 0 && stat(details_path, &sb) == 0) {
	...
	createiprostats(ipro.numAtoms, &ipro_st);
	...
	getresults(&ipro_st, &grid, &ilig, results, rank);
	fprintf(stdout, "Top sphere identified\n");
	fprintf(stdout, "printing...\n");

	fptr = fopen(results_path, "w");
	printresults(results, rank, fptr);
	fclose(fptr);

	fptr = fopen(details_path, "w");
	printdetails(&ipro_st, &grid, results, rank, fptr);
	fclose(fptr);
